...
static QueueHandle_t QueueDriRotenHandle; /*创建编码器队列*/
static TypedefDataRT Data_Input_RT;              /*rot队列的数据*/
...
void RotaryEncoder_IRQ_Callback(void)
{
     ...
     * 防抖
     */
    if(time-pre_time<2000000)  //如果当前的中断于上一个中断时间间隔小于2ms的话就不执行下面的代码
								//两个上升沿之间的时间要比它最大的抖动时间都要长
	{
		pre_time=time;
		return;
	}
	...
	Data_Input_RT.Speed=g_speed;
	xQueueSendToBackFromISR(QueueDriRotenHandle,&Data_Input_RT,NULL);
    pre_time = time;
...
void RotaryEncoder_Init(void)
{
	...
	QueueDriRotenHandle = xQueueCreate(15,sizeof(Data_Input_RT));
}
