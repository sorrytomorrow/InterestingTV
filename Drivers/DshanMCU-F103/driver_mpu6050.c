...
/*创建关于mpu6050的队列句柄*/
TypedefMpu6D QueueMpu6Data;
static QueueHandle_t g_xQueueMpu6050H;
/*返回mpu6050的队列句柄*/
QueueHandle_t Get_QueueMpu6Handle(void)
{
	return g_xQueueMpu6050H;
}
...
int MPU6050_Init(void)
{
	...
	/*创建关于MPU6050的任务队列*/
	g_xQueueMpu6050H = xQueueCreate(15,sizeof(QueueMpu6Data));
...
/*解析数据转化为角度*/ /*int16_t AccY, int16_t AccZ, int16_t GyroX, int16_t GyroY, int16_t GyroZ, */
void MPU6050_ParseData(int16_t AccX,TypedefMpu6D *result)
{
	if (result)
	{
		result->Ax_val = (int32_t)(acos((double)((double)(AccX + MPU6050_X_ACCEL_OFFSET) / 16384.0)) * 57.29577);
	}
}

extern void GetI2cMutex(void);
extern void ReleaseI2cMutex(void);

/*把读取到的MPU6050寄存器里的数据写入到MPU6050的队列中*/
void WDataTQueFromMpu(void)
{
	uint8_t datal, datah;
	int16_t AX;
	int err = 0;
	while(1)
	{
		...
		ReleaseI2cMutex();
		AX=(datah << 8) | datal;
		MPU6050_ParseData(AX,&QueueMpu6Data);
		/*写入队列*/
		xQueueSend(g_xQueueMpu6050H,&QueueMpu6Data,0);
		vTaskDelay(50);
	}
...
//        LCD_PrintSignedVal(12, 4, GyroZ);
		  WDataTQueFromMpu();
		  LCD_PrintSignedVal(0,0,QueueMpu6Data.Ax_val);
    }
}
